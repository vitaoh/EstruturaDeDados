...
int main() {
    ...
    while (1) {
        ...
        if (aeronave[0] == '-' || (aeronave[0] >= '0' && aeronave[0] <= '9')) {
            ...
            if (ponto_cardinal == 0) break;
            continue;
        }
        
        switch (ponto_cardinal) {
            case -1: enfileirar(&oeste, aeronave); break;
            case -3: enfileirar(&norte, aeronave); break;
            case -2: enfileirar(&sul, aeronave); break;
            case -4: enfileirar(&leste, aeronave); break;
        }
    }
